...
docker run --network host frontend-build bash -v $(pwd)/frontend/cypress/:/app/cypress -c "apt-get update && apt-get install -qq xvfb libnss3 libatk1.0 libatk-bridge2.0 libgtk-3.0 libgbm1 libasound2 && yarn run-e2e-tests"
